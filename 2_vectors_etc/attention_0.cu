...
__global__ void attention_2x32(const int n,
    ...
    if (j < n) {
        ...
        // Calculate value: V = wv*x
        // -> Caclulate the value part

        // Calculate query: q = wq*x_i
        // -> Caclulate the query that this thread is dealling with

        __syncthreads();

        // Softmax
        ftype qkt[32], denom = 0.;
        // -> Calcualte the softmax

        // Apply denom and value
        // -> Apply the value to the query and write out.
    }
}
