...
__global__ void attention_2x32(const int n,
    ...
    if (j < n) {
        vtype2 x_l = __ldcs((vtype2 *)(x + 2*i + 64*batch));
        vtype4 wq_l = __ldca((vtype4 *)(wq));
        vtype4 wk_l = __ldca((vtype4 *)(wk));
        vtype4 wv_l = __ldca((vtype4 *)(wv));

        // Calculate key
        k_s[i + 0*32 + bld] = wk_l.x*x_l.x + wk_l.z*x_l.y;
        k_s[i + 1*32 + bld] = wk_l.y*x_l.x + wk_l.w*x_l.y;

        // Calculate value
        v_s[i + 0*32 + bld] = wv_l.x*x_l.x + wv_l.z*x_l.y;
        v_s[i + 1*32 + bld] = wv_l.y*x_l.x + wv_l.w*x_l.y;

        // Calculate query
        ftype q[2];
        q[0] = wq_l.x*x_l.x + wq_l.z*x_l.y;
        q[1] = wq_l.y*x_l.x + wq_l.w*x_l.y;
        ...
        for (int iii=0; iii<2; ++iii) {
            ...
            __stcs(z + iii + 2*i + 64*batch, v * rcp_denom);
        }
    }
}
