...
// -> Update these prototypes to be for a device function
ftype tricomi_approx(const int n, const int k);
ftype olver_approx(const int n, const int k);
...
ftype tricomi_approx(const int n, const int k) { // best for |x| < 0.5
...
// -> Convert to device function
ftype olver_approx(const int n, const int k) { // best for 0.5 <= |x| <=1
    ftype nph = n + 0.5;
...
int main(void) {
    ...
    // Create the stream
    // -> create a stream for this computation
    error_check(cerr, 2);
    ...
    // -> Add call to calculate_theta
    // -> Add call to invert_theta
