...
__constant__ __device__ ftype pi = 3.141592653589793;
// Approximation parameters from JCP 42, pp. 403-405 (1981)
__constant__ __device__ ftype a0 = 0.0682894897349453;
__constant__ __device__ ftype a1 = 0.131420807470708;
__constant__ __device__ ftype a2 = 0.0245988241803681;
__constant__ __device__ ftype a3 = 0.000813005721543268;

__constant__ __device__ ftype b0 = 1.;
__constant__ __device__ ftype b1 = 1.16837242570470;
__constant__ __device__ ftype b2 = 0.200991122197811;
__constant__ __device__ ftype b3 = 0.00650404577261471;
...
__global__ void invert_theta(const int n, const ftype * __restrict__ theta, ftype * __restrict__ x) {
...
__global__ void calculate_theta(const int n, ftype * __restrict__ theta) {
    int k = blockDim.x*blockIdx.x + threadIdx.x;
    int n2 = floor(n * 0.5);
...
__device__ __inline__ ftype tricomi_approx(const int n, const int k) { // best for |x| < 0.5
...
__device__ __inline__ ftype olver_approx(const int n, const int k) { // best for 0.5 <= |x| <=1
    ftype nph = n + 0.5;
...
__device__ __inline__ ftype bessel0_root(const int k) {
    ftype beta = (k - 0.25) * pi;
    ftype beta_sq = beta * beta;

    ftype j0 = a0 + beta_sq*(a1 + beta_sq*(a2 + beta_sq*a3));
    j0 /= beta*(b0 + beta_sq*(b1 + beta_sq*(b2 + beta_sq*b3)));
    j0 += beta;
