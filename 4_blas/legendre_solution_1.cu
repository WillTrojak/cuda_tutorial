...
int main(void) {
    ...
    error_check(cerr, 11);
    cerr = cudaStreamCreate(&stream2);
    error_check(cerr, 12);
    cerr = cudaStreamCreate(&stream3);
    error_check(cerr, 13);

    // Create a cublas handle and set the stream
    cublasHandle_t handle1, handle2, handle3;
    bstat = cublasCreate(&handle1);
    bstat_check(bstat, 14);
    bstat = cublasCreate(&handle2);
    bstat_check(bstat, 15);
    bstat = cublasCreate(&handle3);
    bstat_check(bstat, 16);

    bstat = cublasSetStream(handle1, stream1);
    bstat_check(bstat, 17);
    bstat = cublasSetStream(handle2, stream2);
    bstat_check(bstat, 18);
    bstat = cublasSetStream(handle3, stream3);
    bstat_check(bstat, 19);
    ...
    bstat = cublasSgemm(handle2, CUBLAS_OP_N, CUBLAS_OP_N,  npnts, neles,
                        order+1, &alpha, proj_d, npnts, mode2_d, order+1, &beta,
                        tran_d + 1*nmode, npnts);
    bstat = cublasSgemm(handle3, CUBLAS_OP_N, CUBLAS_OP_N,  npnts, neles,
                        order+1, &alpha, proj_d, npnts, mode3_d, order+1, &beta,
                        tran_d + 2*nmode, npnts);
    cerr = cudaDeviceSynchronize();
    error_check(cerr, 20);
    bstat_check(bstat, 21);
