...
int main(void) {
    ...
    cerr = cudaStreamBeginCapture(stream1, cudaStreamCaptureModeGlobal);
    ...
    cerr = cudaStreamEndCapture(stream1, &graph);
    error_check(cerr, 14);

    // Mkae an executable graph
    cudaGraphExec_t graph_run;
    cerr = cudaGraphInstantiate(&graph_run, graph, 0);
    error_check(cerr, 15);

    // Try running the graph
    cerr = cudaGraphLaunch(graph_run, stream1);
    error_check(cerr, 16);
