...
__global__ void layer_32x32(const int n, const ftype __restrict__ *w,
    ...
    if (j < n) {
        ...
        ftype acc = b[lane];
        for (int i=0; i < 32; i++)
            acc += w[lane + i*32] * x[i + 32*batch];

        // Apply ReLU activation and write to z
        z[lane + 32*batch] = max((ftype)0., acc);
    }
}


// z = sigma(w*x + b)
__global__ void layer_32x32_shared(const int n, const ftype __restrict__ *w,
                                   const ftype __restrict__ *x,
                                   const ftype __restrict__ *b,
                                   ftype __restrict__ *z) {
    int j = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    if (j < n) {
        ...
        ftype acc = b_s[lane];
        for (int i=0; i < 32; i++)
            acc += w_s[lane + i*32] * x[i + 32*batch];
...
__global__ void layer_32x32_shared_x(const int n, const ftype __restrict__ *w,
    ...
    int warp = threadIdx.x / 32;
    int l_batch = warp;
    int n_batchs = blockDim.x / 32;

    int lane = threadIdx.x % warpSize;
    __shared__ ftype x_s[BLOCKDIMX];

    if (k < n) {
        // Load weight into shared memory
        if (warp == 0){
            for (int i=0; i<n_batchs; ++i) {
                x_s[lane + 32*i] = x[lane + 32*(batch + i)];
            }
        }
        __syncthreads();
        ...
        {
            int j = (lane + i) % 32;
            acc += w[lane + 32*j] * x_s[j + 32*l_batch];
        }
...
int main(void) {
    ...
    // Launch the kernel
    layer_32x32<<<blocks_per_grid, threads_per_block>>>(n, w_d, x_d, b_d, z_d);
    ...
    for (int i=0; i < ncycles; ++i) {
        layer_32x32<<<blocks_per_grid, threads_per_block, 0, stream>>>(n, w_d, x_d, b_d, z_d);
    }
    end_time = clock();
    run_time = ((float)(end_time - start_time)) / CLOCKS_PER_SEC;
    printf("Run time (  base  ): %f\n", run_time);

    start_time = clock();
    for (int i=0; i < ncycles; ++i) {
        layer_32x32_shared<<<blocks_per_grid, threads_per_block, 0, stream>>>(n, w_d, x_d, b_d, z_d);
    }
    end_time = clock();
    run_time = ((float)(end_time - start_time)) / CLOCKS_PER_SEC;
    printf("Run time ( shared ) : %f\n", run_time);

    start_time = clock();
    for (int i=0; i < ncycles; ++i) {
        layer_32x32_shared_x<<<blocks_per_grid, threads_per_block, 0, stream>>>(n, w_d, x_d, b_d, z_d);
    }
    end_time = clock();
    run_time = ((float)(end_time - start_time)) / CLOCKS_PER_SEC;
    printf("Run time (shared x): %f\n", run_time);
